...
function fileMapperValidation(data, model, _sd, sNo, validData, errorData, invalidSNo, req, serviceDetailsObj) {
    return new Promise((resolve, reject) => {
        req.query.source = 'fileMapper';
...
function processValidation(arr, batch, model, serviceDetail, validData, errorData, invalidSNo, _req, serviceDetailObj){
    let output = [];
    var arrays = [],
        size = batch;
    while (arr.length > 0) {
    ...
    return arrays.reduce((_p, _c, i) => {
        return _p.then(() => {
            ...
            let validationPromiseArr = _c.map(obj => {
                return fileMapperValidation(obj.data, model, serviceDetail, obj.sNo, validData, errorData, invalidSNo, _req, serviceDetailObj)
            });
    ...
    .then((bufferData)=> {
        ...
        .then(() => {
            ...
            .then(schemaJSON => {
                ...
                return processValidation(serializedValidationData, batch, model, serviceDetail, validData, errorData, invalidSNo, _req, serviceDetailObj);
