...
async function execute() {
    ...
    logger.debug('=======================================');
    let status = await fileTransfersModel.findOneAndUpdate({ fileId: fileId }, { $set: { isHeaderProvided, headerMapping, status: 'Validating' } });

    /**---------- After Response Process ------------*/
    startTime = Date.now();
    status = await model.deleteMany({ fileId });
    ...
    let batchPromise = await Promise.all(batch.map((e) => model.insertMany(e)));
    batch = null;
    serializedData = null;
    batchPromise = null;
    endTime = Date.now();
    ...
    status = await model.updateMany({ fileId, 'data._id': { $in: conflictIds } }, { $set: { conflict: true } });
    conflictDocs = null;
    conflictIds = null;
    endTime = Date.now();
    logger.debug('=======================================');
    ...
    batchPromise = await batch.reduce((prev, items) => {
        return prev.then(() => {
