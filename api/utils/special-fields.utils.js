...
function hasPermissionForGET(req, permissions) {
	...
	if (_.intersection(['ADMIN_SRVC2002'], permissions).length > 0) {
		return true;
	}
	if (_.intersection(["P1927354248","P6727857889"], permissions).length > 0) {
		return true;
	}
	if (_.intersection(["C6329029760"], permissions).length > 0) {
		return true;
	}
	return false;
}
...
function hasPermissionForPUT(req, permissions) {
	...
	if (_.intersection(["P1927354248"], permissions).length > 0) {
		return true;
	}
	return false;
}
...
hasWFPermissionFor['Test'] = function(req, permissions) {
	if (process.env.SKIP_AUTH == 'true' || process.env.SKIP_AUTH == 'TRUE') {
		return true;
	}
	if (_.intersection(['ADMIN_SRVC2002'], permissions).length > 0) {
...
async function getDynamicFilter(req, data) {
	let filter;
	let allFilters = [];
	if (_.intersection(['ADMIN_SRVC2002'], (req.user && req.user.appPermissions ? req.user.appPermissions : [])).length > 0) {
		return null;
	}
	if (process.env.SKIP_AUTH == 'true' || process.env.SKIP_AUTH == 'TRUE') {
		return null;
